...
class SimDeep(DeepBase):
    ...
    def predict_labels_using_external_labels(self, labels, labels_proba):
        ...
        df = pd.DataFrame({'labels': self.labels, 'nbdays': nbdays, 'isdead': isdead})
        ...
        df_proba = pd.DataFrame({'labels_proba': self.labels_proba.T[0], 'nbdays': nbdays, 'isdead': isdead})
        pvalue_proba = coxph(df_proba, time_col='nbdays', event_col='isdead', features=['labels_proba'],
                             seed=self.seed,
                             use_r_packages=self.use_r_packages,
                             isfactor=False)
        ...
        if not self.is_model_loaded:
            if self.alternative_embedding is not None:
                ...
                try:
                    ...
                    print('Autoencoder construction failed: {0}'.format(e))
                    print('Falling back to alternative embedding: using raw features')
                    self.use_autoencoders = False

        self.look_for_survival_nodes()
        keys = list(self.encoder_array.keys())
        if not keys:
            keys = list(self.matrix_train_array.keys())
        self.training_omic_list = keys
        self.predict_labels()
    ...
    def _compute_test_coxph(self,
        ...
        df = pd.DataFrame({'labels': labels, 'nbdays': nbdays, 'isdead': isdead})
        if metadata_mat is not None:
            df = pd.concat([df, metadata_mat.reset_index(drop=True)], axis=1)
            features = ['labels'] + metadata_mat.columns.tolist()
        else:
            features = ['labels']
        ...
        # Parse fname_base to determine data source (e.g., 'KM_plot_test', 'KM_plot_full')
        # Common patterns in fname_base: 'KM_plot_test', 'KM_plot_full', 'KM_plot_test_fold'
        # We can use this to construct the plot title prefix
        plot_title_prefix = fname_base.replace('KM_plot_', '')
        
        pvalue = coxph(
            df, time_col='nbdays', event_col='isdead', features=features,
            isfactor=False,
            do_KM_plot=self.do_KM_plot,
            png_path=self.path_results,
            seed=self.seed,
            use_r_packages=self.use_r_packages,
            metadata_mat=metadata_mat,
            fig_name='{0}_{1}_{2}_seed{3}'.format(self.project_name, fname_base, timestamp, self.seed),
            plot_title='{0} / labels / seed {1}'.format(plot_title_prefix, self.seed))

        if self.verbose:
            print('Cox-PH p-value (Log-Rank) for inferred labels: {0}'.format(pvalue))

        df_proba = pd.DataFrame({'labels_proba': labels_proba.T[0], 'nbdays': nbdays, 'isdead': isdead})
        if metadata_mat is not None:
            df_proba = pd.concat([df_proba, metadata_mat.reset_index(drop=True)], axis=1)
            features_proba = ['labels_proba'] + metadata_mat.columns.tolist()
        else:
            features_proba = ['labels_proba']

        # Note: We generally don't plot KM for proba directly in this call unless forced, 
        # but if we did, we would want consistent naming. 
        # However, the original code sets do_KM_plot=False for proba here.
        # If we wanted to enable it, we would pass do_KM_plot=self.do_KM_plot
        
        pvalue_proba = coxph(
            df_proba, time_col='nbdays', event_col='isdead', features=features_proba,
            isfactor=False,
            do_KM_plot=False, # Kept as False per original logic
            png_path=self.path_results,
            seed=self.seed,
            use_r_packages=self.use_r_packages,
            metadata_mat=metadata_mat,
            fig_name='{0}_{1}_proba_{2}_seed{3}'.format(self.project_name, fname_base, timestamp, self.seed),
            plot_title='{0} / proba / seed {1}'.format(plot_title_prefix, self.seed))
    ...
    def predict_labels(self):
        ...
        if hasattr(self.cluster_method, 'fit_predict'):
            ...
            self.clustering = KMeans(n_clusters=self.nb_clusters, n_init=100, random_state=self.seed)

        elif self.cluster_method == 'mixture':
            # 添加random_state确保可复现性
            mixture_params = self.mixture_params.copy()
            if self.seed is not None:
                mixture_params['random_state'] = self.seed
    ...
    def _order_labels_according_to_survival(self, labels):
        ...
        for label in set(labels_old):
            mean = surv_median(np.zeros(np.sum(labels_old == label)),
                             days[labels_old == label],
                             dead[labels_old == label])
    ...
    def _look_for_prediction_nodes(self, key):
        ...
        score_list = list(filter(lambda x: not np.isnan(x[1]), score_list))
