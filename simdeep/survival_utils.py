...
def load_data_from_tsv(use_transpose=USE_INPUT_TRANSPOSE, **kwargs):
    ...
    if isinstance(f_name, (list, tuple)):
        if use_transpose:
            return _load_data_from_tsv_transposee_3d(**kwargs)
        else:
            return _load_data_from_tsv_3d(**kwargs)
    if use_transpose:
...
def _load_data_from_tsv_3d(
    ...
    matrices = []
    sample_ids_ref = None
    feature_ids_ref = None
    for f in f_name:
        sids, fids, mat = _load_data_from_tsv(
            f_name=f,
            key=key,
            path_data=path_data,
            f_type=f_type,
        ...
        if sample_ids_ref is None:
            sample_ids_ref = sids
            feature_ids_ref = fids
        else:
            assert(sample_ids_ref == sids)
            assert(feature_ids_ref == fids)
        matrices.append(mat)
    f_matrix = np.stack(matrices, axis=2)
    return sample_ids_ref, feature_ids_ref, f_matrix
...
def _load_data_from_tsv_transposee_3d(
        f_name,
        key,
        path_data=PATH_DATA,
        f_type=float,
    ...
    feature_ids_ref = None
    for f in f_name:
        sids, fids, mat = _load_data_from_tsv_transposee(
            f_name=f,
            key=key,
            path_data=path_data,
            f_type=f_type,
