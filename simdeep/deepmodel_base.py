...
import random
# 移除模块级别的硬编码seed，改为在类初始化时根据传入的seed设置
# 这样可以确保用户传入的seed能够正确生效
# random.seed(2020)  # 已移除，改为在类中设置
...
# 移除模块级别的硬编码seed，改为在类初始化时根据传入的seed设置
# np.random.seed(2020)  # 已移除，改为在类中设置
# if set_random_seed is not None:
#     set_random_seed(2020)  # 已移除，改为在类中设置
...
class DeepBase(object):
    ...
    def _create_autoencoder(self, matrix_train, key, matrix_out=None):
        ...
        if not KERAS_AVAILABLE:
            raise Exception('Keras/TensorFlow not installed: autoencoder cannot be constructed')
        if self.verbose:
    ...
    def _add_dense_layer(self, model, shape, dim, name=None):
        ...
        """
        if not KERAS_AVAILABLE:
            raise Exception('Keras/TensorFlow not installed: layer cannot be added')
        input_dim = None
    ...
    def compile_models(self):
        ...
        """
        if not KERAS_AVAILABLE:
            raise Exception('Keras/TensorFlow not installed: cannot compile models')
    ...
    def fit_autoencoders(self, objective=None):
        ...
        """
        if not KERAS_AVAILABLE:
            raise Exception('Keras/TensorFlow not installed: cannot fit models')
        for key in self.model_array:
            model = self.model_array[key]
    ...
    def encoder_predict(self, key, matrix):
        ...
        """
        if not KERAS_AVAILABLE:
            raise Exception('Keras/TensorFlow not installed: cannot predict encoder outputs')
    ...
    def _define_encoders(self):
        ...
        """
        if not KERAS_AVAILABLE:
            raise Exception('Keras/TensorFlow not installed: cannot define encoders')
        for key in self.model_array:
            model = self.model_array[key]
            matrix_train = self.matrix_train_array[key]
