...
class LoadData():
    ...
    def load_matrix_test(self, normalization=None):
        ...
        expanded = []
        for base_key in self.test_tsv:
            f_val = self.test_tsv[base_key]
            if isinstance(f_val, (list, tuple)):
                for i, f_name in enumerate(f_val):
                    subkey = '{0}#{1}'.format(base_key, i)
                    expanded.append((base_key, subkey, f_name))
            else:
                expanded.append((base_key, base_key, f_val))

        for base_key, subkey, f_name in expanded:
            sample_ids, feature_ids, matrix = load_data_from_tsv(
                f_name=f_name,
                key=subkey,
                path_data=self.path_data)

            feature_ids_ref = self.feature_array[subkey]
            matrix_ref = self.matrix_array[subkey].copy()
            ...
            self.feature_test_array[subkey] = list(common_features)
            ...
            matrix_ref, matrix_test = self.transform_matrices(
                matrix_ref, matrix_test, subkey, normalization=normalization)

            self._define_test_features(subkey, normalization)

            self.matrix_test_array[subkey] = matrix_test
            self.matrix_ref_array[subkey] = matrix_ref
            self.feature_ref_array[subkey] = self.feature_test_array[subkey]
            self.feature_ref_index[subkey] = {feat: pos for pos, feat in enumerate(common_features)}

            self._define_ref_features(subkey, normalization)
    ...
    def load_array(self):
        ...
        to_load = []
        for base_key in self.data_type:
            f_val = self.training_tsv[base_key]
            if isinstance(f_val, (list, tuple)):
                for i, f_name in enumerate(f_val):
                    subkey = '{0}#{1}'.format(base_key, i)
                    to_load.append((subkey, f_name))
            else:
                to_load.append((base_key, f_val))

        first_key, first_file = to_load[0]
        ...
        if self.verbose:
            print('{0} loaded of dim:{1}'.format(first_file, matrix.shape))

        self.feature_array[first_key] = feature_ids
        self.matrix_array[first_key] = matrix

        for data, f_name in to_load[1:]:
            sample_ids, feature_ids, matrix = load_data_from_tsv(
                f_name=f_name,
                key=data,
                path_data=self.path_data)
            ...
            if self.sample_ids != sample_ids:
                ...
                for data2 in list(self.matrix_array.keys()):
